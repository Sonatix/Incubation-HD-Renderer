...
// Counts triangles actually drawn since the last grBufferSwap. Used there to
// skip presenting empty (pure-2D) frames so the DirectDraw/GDI 2D layer is not
// flickered by an empty GL buffer swap. Always compiled (unlike the OGL_DEBUG
// FrameTriangles field).
long g_FrameDrawnTriangles = 0;
...
void RenderDrawTriangles( void )
{
    ...
    g_FrameDrawnTriangles += OGLRender.NumberOfTriangles;
    ...
    // EXPERIMENTAL: fake bump/normal mapping, active only for the target texture
    // and only single-textured draws (bump3d.cpp). No-op otherwise.
    extern bool bump3d_begin( void );
    extern void bump3d_end( void );
    bool bumped = ( !use_two_tex ) && bump3d_begin( );
