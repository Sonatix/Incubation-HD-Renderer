...
// GLSL sharpen pass for the composited 2D layer (sharp2d.cpp)
extern bool sharp2d_begin( int texSize );
extern void sharp2d_end( void );
...
{ 
    ...
    if ( dwType & 1 )
    {
        ...
        if ( maxx >= minx )
        {
            ...
            // GL_LESS 0.5 (was GL_EQUAL 0.0): drawn pixels have alpha 0, the
            // color-keyed transparent ones alpha 1. With the LFB texture now
            // bilinear, edge texels interpolate to ~0.5; LESS 0.5 splits them
            // cleanly instead of eroding every keyed edge.
            ...
            bool sharp = sharp2d_begin( Glide.LFBTextureSize );
